...
#编码的时候相应类在分子，统计分析的时候符号应该为负
    ...
    def cmt_iv(self,limited_value = -np.inf,print_flag = False):
        times = 0
        iv_list = []
        iv_dict = {}
    ...
    def cmt_ks(self,limited_value = -np.inf,print_flag = False):
        times = 0
        ks_list = []
        ks_dict = {}
        for x in self.col_list:
            ...
            if ks_v > limited_value:
                ...
                times+=1
        print('the number of bigger then 0.02 if %f'%times)
        return ks_list,ks_dict
    
        
    def filtered_df(self,ks_limited = 0.02,iv_limited = 0.02):
        iv = self.cmt_iv(iv_limited)
        ks = self.cmt_ks(ks_limited)
        return self.df[iv_list[0]+ks_list[0]+[self.col]]
#####################################################################
